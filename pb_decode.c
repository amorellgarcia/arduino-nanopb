...
bool checkreturn pb_delim_decode_noinit(pb_istream_t *stream, const pb_field_t fields[], void *dest_struct)
{
    uint32_t length;
    if (pb_decode_varint32(stream, &length))
    {
        size_t bytes_left = stream->bytes_left - length;

        stream->bytes_left = length;
        if (pb_decode_noinit(stream, fields, dest_struct))
        {
            stream->bytes_left = bytes_left;
            return true;
        }
        ...
        PB_RETURN_ERROR(stream, "decoding message length");

    return false;
}

bool checkreturn pb_delim_decode(pb_istream_t *stream, const pb_field_t fields[], void *dest_struct)
{
    pb_message_set_to_defaults(fields, dest_struct);
    return pb_delim_decode_noinit(stream, fields, dest_struct);
}
